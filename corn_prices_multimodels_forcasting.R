...
corn_prices_monthly_raw <- tq_get(x = "PMAIZMTUSDM", 
                                 # x = "GASREGCOVM",
                                 get  = "economic.data", 
                                 from = "1990-01-01", to = "2020-12-31") 

corn_prices_monthly_raw

# any NAs?
summary(corn_prices_monthly_raw$price)
...
corn_prices_monthly <- corn_prices_monthly_raw %>%
...
corn_prices_monthly %>%
  ...
  labs(title = "Corn Prices, Monthly", x = "", y = "USD") +
...
corn_prices_quarterly <- corn_prices_monthly %>%
  tq_transmute(mutate_fun = to.period, period = "quarters") 
corn_prices_quarterly

# plot quaterly data
corn_prices_quarterly %>%
  ggplot(aes(x = date, y = price)) +
  geom_line(color = palette_light()[[1]], size = 1) +
  labs(title = "Corn Prices, Quarterly", x = "", y = "USD") +
  scale_y_continuous(labels = scales::dollar) +
...
corn_prices_quarterly_ts <- corn_prices_quarterly %>% 
  tk_ts(select = -date, start = c(1990, 3), freq = 4)
corn_prices_quarterly_ts
...
models_tbl_fcast_tidy %>%
  ...
  geom_line(size = 1) +
  facet_wrap(~f, nrow = 3) +
  labs(title = "Corn Price Forecasts",
